...
export default function ListLayout({ users, initialDisplayUsers }) {
  ...
  const filteredDisplayUser = users.filter((frontMatter) => {
    const searchContent = frontMatter.name + frontMatter.email + frontMatter.role
    return searchContent.toLowerCase().includes(searchValue.toLowerCase())
  })

  // If initialDisplayPosts exist, display it if no searchValue is specified
  const displayUsers =
    initialDisplayUsers.length > 0 && !searchValue ? initialDisplayUsers : filteredDisplayUser
